...
use crate::{
    gpu::vk_gpu::Gpu,
    instruction::{
        Instruction, gpu_operations::GPUOperation, max::f32_f32_f32_cpu::f32_f32_f32_cpu,
    },
    tensor::TensorDesc,
...
impl Instruction for MaxInstruction {
    ...
    fn pick_gpu_operation(&self, cm: &ComputeManager) -> Result<Option<GPUOperation>, VKMLError> {
        ...
        if src1_dtype != src2_dtype || src1_dtype != dst_dtype {
            return Err(VKMLError::Instruction(format!(
                "GPU Max unimplemented for mixed DataType src1:{:?}, src2:{:?}, dst:{:?}",
    ...
    fn record_into_command_buffer(
        ...
        let op_name = match op {
            Some(GPUOperation::Maximum) => GPUOperation::Maximum,
            _ => {
                return Err(VKMLError::Instruction(format!(
                    "Invalid GPUOperation {:?} for Max",
                    op
                )));
            }
        };
