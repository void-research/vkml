...
use crate::{
    gpu::Gpu,
    instruction::{GpuShader, Instruction, min::f32_f32_f32_cpu::f32_f32_f32_cpu},
    ...
    fn can_run_on(&self, target: &ComputeTarget, cm: &ComputeManager) -> Result<bool, VKMLError> {
        ...
        if TensorDesc::broadcast_shape(src1_desc.dims(), src2_desc.dims()).is_none() {
            return Err(VKMLError::Instruction(format!(
                "Min instruction {:?}: cannot broadcast shapes {:?} and {:?}",
                self,
                src1_desc.dims(),
                src2_desc.dims()
            )));
        ...
        if dst_desc.dims().len() > 8 {
            return Err(VKMLError::Instruction(format!(
                "Min instruction {:?}: tensor rank exceeds max supported 8 (got {})",
                self,
                dst_desc.dims().len()
            )));
        }
        ...
        match target {
            ComputeTarget::Gpu(gpu) => {
                ...
                let compatible = src1_desc.data_type() == dst_dtype
                    ...
                    && GpuShader::Minimum.info().can_run_on(gpu, dst_dtype);
                Ok(compatible)
            }
            ComputeTarget::Cpu => {
